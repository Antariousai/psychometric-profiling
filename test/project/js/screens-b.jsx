// screens-b.jsx — Assessment, Scoring, Result screens
...
function AssessmentScreen({ ctx }) {
  ...
  const T = window.T, PERSONAS = window.PERSONAS, bn = window.bn;
  const BilingualLabel = window.BilingualLabel, Chip = window.Chip, BrandHeader = window.BrandHeader;
  const FreyaOrb = window.FreyaOrb;

  const [idx, setIdx] = uSb(0);
  const [answers, setAnswers] = uSb(() => ctx.answers || {});
  const startAt = uRb(Date.now());
  const firstRender = uRb(true);

  uEb(() => { startAt.current = Date.now(); }, [idx]);
  uEb(() => {
    if (firstRender.current) { firstRender.current = false; return; }
    ctx.setAnswers(answers);
  }, [answers]);
    ...
    <div style={{ background: T.cream, minHeight: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '10px 16px 0', background: '#fff' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 2px 12px', gap: 12 }}>
          <span style={{ fontFamily: T.fBn, fontSize: 11, color: dim.color, fontWeight: 700, lineHeight: 1.5, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{dim.bn}</span>
          <span style={{ fontFamily: T.fMono, fontSize: 10, color: T.ink3, fontWeight: 700, flexShrink: 0 }}>{bn(Math.round(progress * 100))}%</span>
        </div>
      </div>

      {/* Question body */}
      <div style={{ flex: 1, padding: '20px 16px 120px', background: T.cream, overflowY: 'auto' }}>
        <div style={{
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 18 }}>
          <div style={{
            ...
            fontFamily: T.fBn, fontSize: 11, color: T.ink2, fontWeight: 700,
        ...
        {q.type === 'scale'
          ? <ScaleInputB q={q} answered={answered} onAnswer={answer} T={T} bn={bn} />
          : <OptionListB q={q} answered={answered} onAnswer={answer} T={T} />}
      </div>

      {/* Bottom bar */}
      <div style={{
        position: 'sticky', bottom: 0, padding: '10px 16px', background: '#fff',
        ...
        }}>⚡ ডেমো: সব পূরণ করুন</button>
      </div>
    </div>
  );
}

function OptionListB({ q, answered, onAnswer, T }) {
  return (
        ...
        return (
          <button key={i} onClick={() => onAnswer(i)} style={{
            ...
            <div style={{
              ...
              background: sel ? T.teal : T.cream2, color: sel ? '#fff' : T.ink3,
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontFamily: T.fMono, fontSize: 13, fontWeight: 700, flexShrink: 0,
            }}>{sel ? '✓' : String.fromCharCode(65 + i)}</div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontFamily: T.fBn, fontSize: 14, color: T.ink, fontWeight: sel ? 700 : 600, lineHeight: 1.55 }}>{opt.bn}</div>
              <div style={{ fontFamily: T.fBody, fontSize: 10, color: T.ink3, fontStyle: 'italic', marginTop: 4, lineHeight: 1.4 }}>{opt.en}</div>
            </div>
          </button>
        );
      })}
...
function ScaleInputB({ q, answered, onAnswer, T, bn }) {
  const [hover, setHover] = uSb(null);
  const current = answered?.value ?? null;
  const max = q.scale.max, min = q.scale.min;
  return (
    <div style={{ background: '#fff', border: `1.5px solid ${T.border}`, borderRadius: 14, padding: '18px 16px 14px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 14 }}>
        <div>
          <div style={{ fontFamily: T.fBn, fontSize: 11, color: T.ink3, fontWeight: 700 }}>{q.scale.minBn}</div>
          <div style={{ fontFamily: T.fMono, fontSize: 9, color: T.ink4, marginTop: 2 }}>{q.scale.minEn}</div>
        </div>
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontFamily: T.fBn, fontSize: 11, color: T.ink3, fontWeight: 700 }}>{q.scale.maxBn}</div>
          <div style={{ fontFamily: T.fMono, fontSize: 9, color: T.ink4, marginTop: 2 }}>{q.scale.maxEn}</div>
        </div>
      </div>
        ...
        {Array.from({ length: max - min + 1 }).map((_, i) => {
          ...
          return (
            <button key={val} onClick={() => onAnswer(val)}
              onMouseEnter={() => setHover(val)} onMouseLeave={() => setHover(null)}
              style={{
                flex: 1, aspectRatio: '1', borderRadius: 14, border: 'none',
                background: on ? c : `${c}20`, color: on ? '#fff' : c,
...
function FreyaHint({ q, dim, onClose }) {
  const T = window.T, FreyaOrb = window.FreyaOrb;
  const hints = {
...
function ScoringScreen({ ctx }) {
  const T = window.T, PERSONAS = window.PERSONAS, FreyaOrb = window.FreyaOrb, bn = window.bn;
  ...
  uEb(() => {
    if (step >= messages.length) { setTimeout(() => ctx.go('result'), 500); return; }
  ...
  return (
    <div style={{
      background: T.navy, minHeight: '100%', color: '#fff', display: 'flex', flexDirection: 'column',
...
function ResultScreen({ ctx }) {
  const T = window.T, PERSONAS = window.PERSONAS, DIMENSIONS = window.DIMENSIONS, QUESTIONS = window.QUESTIONS;
  const bn = window.bn, fmtTk = window.fmtTk, computeScore = window.computeScore;
  const BilingualLabel = window.BilingualLabel, Chip = window.Chip, BrandHeader = window.BrandHeader;
  const FreyaOrb = window.FreyaOrb;
  ...
  const result = uMb(() => {
    if (Object.keys(ctx.answers).length === 0) {
      ...
      return {
        overall: p.overall, rating: p.rating,
        risk: p.rating === 'A' ? 'Low' : p.rating === 'B' ? 'Moderate' : p.rating === 'C' ? 'Elevated' : 'High',
        ...
        dimScores: DIMENSIONS.map((d, i) => ({
          ...d, pct: p.rating === 'A' ? [85, 72, 88, 78, 82, 75, 90][i] : p.rating === 'B' ? [78, 65, 70, 72, 68, 60, 82][i] : [45, 38, 42, 58, 40, 52, 48][i]
        })),
      };
    }
  ...
  return (
    <div style={{ background: T.cream, minHeight: '100%' }}>
      ...
      {/* Hero */}
      <div style={{ background: `linear-gradient(160deg, ${T.navy} 0%, ${T.navy2} 100%)`, padding: '22px 20px 20px', color: '#fff', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: -80, right: -80, width: 260, height: 260, borderRadius: '50%', background: `radial-gradient(circle, ${scoreColor}33, transparent 70%)` }} />
        <div style={{ display: 'flex', alignItems: 'center', gap: 13, marginBottom: 20, position: 'relative' }}>
          <div style={{ width: 48, height: 48, borderRadius: 12, background: applicant.tint, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: T.fBn, fontSize: 18, fontWeight: 900, color: '#fff' }}>{applicant.avatar}</div>
          <div style={{ flex: 1 }}>
          ...
          <div style={{ padding: '5px 12px', background: `${scoreColor}22`, border: `1px solid ${scoreColor}`, borderRadius: 20, fontFamily: T.fMono, fontSize: 11, color: scoreColor, fontWeight: 700 }}>{result.risk.toUpperCase()}</div>
        </div>
        ...
        <div style={{ marginTop: 14, height: 5, background: 'rgba(255,255,255,0.1)', borderRadius: 3, overflow: 'hidden', position: 'relative' }}>
          ...
          {[250, 500, 750].map(m => <div key={m} style={{ position: 'absolute', left: `${m / 10}%`, top: 0, width: 1, height: 5, background: 'rgba(255,255,255,0.3)' }} />)}
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 5, fontFamily: T.fMono, fontSize: 8, color: 'rgba(255,255,255,0.4)' }}>
          <span>D · HIGH</span><span>C</span><span>B</span><span>A · LOW</span>
        </div>
      </div>
      ...
      <div style={{ padding: '16px', background: T.cream }}>
        {tab === 'summary' && <ResultSummary result={result} applicant={applicant} recLoanAmt={recLoanAmt} emi={emi} T={T} bn={bn} fmtTk={fmtTk} FreyaOrb={FreyaOrb} BilingualLabel={BilingualLabel} Chip={Chip} />}
        {tab === 'dims' && <ResultDims result={result} T={T} bn={bn} BilingualLabel={BilingualLabel} />}
        {tab === 'flags' && <ResultFlags result={result} T={T} bn={bn} Chip={Chip} BilingualLabel={BilingualLabel} />}
        {tab === 'loan' && <ResultLoan result={result} applicant={applicant} recLoanAmt={recLoanAmt} emi={emi} T={T} bn={bn} fmtTk={fmtTk} BilingualLabel={BilingualLabel} Chip={Chip} />}
      </div>
      ...
      <div style={{ padding: '0 16px 24px', display: 'flex', gap: 10 }}>
        ...
        <button style={{
          ...
          fontFamily: T.fBn, fontSize: 13, fontWeight: 700, cursor: 'pointer', boxShadow: `0 4px 14px ${T.teal}50`,
...
function ResultSummary({ result, applicant, recLoanAmt, emi, T, bn, fmtTk, FreyaOrb, BilingualLabel, Chip }) {
  const freyaText = {
      ...
      <div style={{ background: T.navy, borderRadius: 14, padding: 14, color: '#fff', marginBottom: 12 }}>
        <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontFamily: T.fBn, fontSize: 12.5, color: 'rgba(255,255,255,0.9)', lineHeight: 1.7 }}>{freyaText[result.rating]}</div>
          </div>
        </div>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 12 }}>
        {[
          { label: { bn: 'সুপারিশকৃত ঋণ', en: 'Recommended' }, val: fmtTk(recLoanAmt), sub: `${bn(result.tenure)} মাসে`, color: T.teal },
          { label: { bn: 'মাসিক কিস্তি', en: 'Monthly EMI' }, val: fmtTk(emi), sub: `${bn(result.tenure)} মাস`, color: T.gold },
          { label: { bn: 'ঝুঁকি স্তর', en: 'Risk tier' }, val: result.risk, sub: `Rating ${result.rating}`, color: result.rating === 'A' ? T.green : result.rating === 'B' ? T.teal : result.rating === 'C' ? T.amber : T.coral },
          { label: { bn: 'পতাকা', en: 'Flags raised' }, val: bn(result.flags.length), sub: result.flags.length === 0 ? 'পরিষ্কার' : 'পর্যালোচনা', color: result.flags.length === 0 ? T.green : T.coral },
        ].map((m, i) => (
          <div key={i} style={{ background: '#fff', border: `1px solid ${T.border}`, borderLeft: `3px solid ${m.color}`, borderRadius: 12, padding: '11px 13px' }}>
            <div style={{ fontFamily: T.fBn, fontSize: 10, color: T.ink3, fontWeight: 600, marginBottom: 3 }}>{m.label.bn}</div>
            <div style={{ fontFamily: T.fHead, fontSize: 18, fontWeight: 900, color: m.color, lineHeight: 1.1 }}>{m.val}</div>
            <div style={{ fontFamily: T.fMono, fontSize: 8.5, color: T.ink4, marginTop: 3, letterSpacing: 0.2 }}>{m.sub}</div>
          </div>
        ))}
      </div>
      <div style={{ background: '#fff', border: `1px solid ${T.border}`, borderRadius: 14, padding: 14 }}>
        ...
        {[
          ...
          <div key={i} style={{ display: 'flex', gap: 10, padding: '10px 0', borderTop: i > 0 ? `1px solid ${T.border}` : 'none' }}>
            <div style={{ width: 22, height: 22, borderRadius: 6, background: T.goldBg, color: T.gold, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: T.fMono, fontSize: 10, fontWeight: 700, flexShrink: 0 }}>{bn(i + 1)}</div>
            <div style={{ flex: 1 }}>
...
function ResultDims({ result, T, bn, BilingualLabel }) {
    ...
    <div>
      <div style={{ background: '#fff', border: `1px solid ${T.border}`, borderRadius: 14, padding: 14, marginBottom: 12 }}>
        ...
        <svg width={size} height={size} style={{ display: 'block', margin: '0 auto' }}>
          {[0.25, 0.5, 0.75, 1].map(f => <circle key={f} cx={cx} cy={cy} r={r * f} fill="none" stroke={T.border} strokeDasharray={f === 1 ? 'none' : '2,3'} />)}
          {result.dimScores.map((d, i) => {
            ...
            return (
              <g key={d.id}>
                ...
                <text x={lx} y={ly} textAnchor="middle" dominantBaseline="middle" fontFamily={T.fBn} fontSize={10} fontWeight={700} fill={d.color}>{d.bn.split(' ')[0]}</text>
              </g>
            );
          })}
          <polygon points={polygon} fill={`${T.teal}33`} stroke={T.teal} strokeWidth={2} />
          {points.map((p, i) => <circle key={i} cx={p.x} cy={p.y} r={4} fill={p.d.color} stroke="#fff" strokeWidth={2} />)}
        </svg>
      </div>
...
function ResultFlags({ result, T, bn, Chip, BilingualLabel }) {
  if (result.flags.length === 0) return (
    <div style={{ background: '#fff', border: `1px solid ${T.green}40`, borderRadius: 14, padding: 24, textAlign: 'center' }}>
      ...
      <div style={{ fontFamily: T.fBn, fontSize: 12, color: T.ink3, marginTop: 8, lineHeight: 1.55 }}>উত্তরে কোনো অসামঞ্জস্য বা সময়গত সন্দেহ পাওয়া যায়নি।</div>
    </div>
  );
  ...
  return (
    <div>
      <div style={{ background: `${T.coral}10`, border: `1px solid ${T.coral}40`, borderLeft: `3px solid ${T.coral}`, borderRadius: 12, padding: '12px 14px', marginBottom: 12 }}>
        ...
        <div style={{ fontFamily: T.fBn, fontSize: 11, color: T.ink2, lineHeight: 1.55 }}>এগুলো প্রত্যাখ্যানের কারণ নয় — সাক্ষাৎকারে অতিরিক্ত যাচাই প্রয়োজন।</div>
      </div>
...
function ResultLoan({ result, applicant, recLoanAmt, emi, T, bn, fmtTk, BilingualLabel, Chip }) {
  return (
    <div>
      ...
      <div style={{ background: '#fff', border: `1px solid ${T.border}`, borderRadius: 14, padding: 14 }}>
        ...
        <div style={{ fontFamily: T.fBn, fontSize: 12, color: T.ink2, lineHeight: 1.8 }}>
          {[
            result.overall >= 650 ? `• মোট স্কোর ${bn(result.overall)} — ভালো জায়গায় আছে` : `• মোট স্কোর ${bn(result.overall)} — সাবধানে শুরু করি`,
...
Object.assign(window, { AssessmentScreen, ScoringScreen, ResultScreen, FreyaHint });
