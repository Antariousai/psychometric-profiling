// App.jsx — Antarious Psychometric Profiling (PKSF) — main React app
// All screens + state + Tweaks. Imports QUESTIONS, DIMENSIONS from questions.jsx.
...
const T = {
  ...
  teal:   '#2EC4B6',
  teal2:  '#1A9D91',
  tealBg: 'rgba(46,196,182,0.10)',
  gold:   '#B5874F',
  gold2:  '#D4A855',
  goldBg: 'rgba(181,135,79,0.10)',
  cream:  '#F7F6F2',
  cream2: '#F0EDE4',
  border: '#E2DED6',
  border2:'#D0CBC0',
  ...
  fBn:    "'Noto Sans Bengali', 'SolaimanLipi', 'Hind Siliguri', sans-serif",
...
const PERSONAS = {
  nasrin: {
    id:'nasrin', name:'নাসরিন বেগম', nameEn:'Nasrin Begum',
    age:34, gender:'F', village:'কমলগঞ্জ, সিলেট', villageEn:'Kamalganj, Sylhet',
    ...
    avatar:'N', tint:'#C2694F',
  },
  rafiq: {
    id:'rafiq', name:'রফিক উদ্দিন', nameEn:'Rafiq Uddin',
    age:42, gender:'M', village:'ভরুয়াখালী, কক্সবাজার', villageEn:'Bharuakhali, Cox\'s Bazar',
    ...
    avatar:'R', tint:'#5E8C41',
  },
  shima: {
    id:'shima', name:'শিমা আক্তার', nameEn:'Shima Akhter',
    age:28, gender:'F', village:'রংপুর সদর', villageEn:'Rangpur Sadar',
...
// Calculate creditworthiness from answers
function computeScore(answers, questions, dimensions) {
  ...
  questions.forEach(q => {
    ...
    if (q.type === 'scale') {
      ...
      max = 5;
      // social-desirability: max value + very fast
  ...
  questions.forEach(q => {
    ...
    // crude: both should lean high or both low
  ...
  // loan recommendation
  let recLoan, rating, tenure, emi, risk;
  if (overall >= 780) { rating='A'; recLoan=null; tenure=24; risk='Low'; }
  else if (overall >= 650) { rating='B'; recLoan=null; tenure=18; risk='Moderate'; }
  else if (overall >= 500) { rating='C'; recLoan=null; tenure=12; risk='Elevated'; }
  else { rating='D'; recLoan=null; tenure=9; risk='High'; }
...
function BilingualLabel({ bn: bnTxt, en, sizeBn=15, sizeEn=11, color=T.ink, enColor=T.ink3, align='left', weight=600, style }) {
  return (
    <div style={{textAlign:align, ...style}}>
      ...
      {en && <div style={{fontFamily:T.fBody, fontSize:sizeEn, fontWeight:500, color:enColor, letterSpacing:0.2, marginTop:5, lineHeight:1.35}}>{en}</div>}
    </div>
  );
}
...
function Chip({children, color=T.teal, bg, size=10, style}) {
  return <span style={{
    ...
    fontFamily:T.fMono, fontSize:size, fontWeight:700, letterSpacing:0.5,
    ...style,
  }}>{children}</span>;
}
...
// Antarious-style orb for Freya — uses the real Freya coin logo
function FreyaOrb({ size=44, pulse=true, style }) {
  return (
    <div style={{
      width:size, height:size, borderRadius:'50%',
      backgroundImage:'url(assets/freya-coin.png)',
      backgroundSize:'115% 115%', backgroundPosition:'center',
...
// Antarious logo (chooses white or dark variant)
function AntariousLogo({ variant='dark', height=22, style }) {
  ...
  return (
    <img src={src} alt="Antarious AI" style={{height, width:'auto', display:'block', ...style}}/>
  );
}

// ───────────────────────── Screens start here ─────────────────────────
// Each screen receives (ctx) with state + nav.
    ...
    <div style={{background:T.navy, color:'#fff', minHeight:'100%', display:'flex', flexDirection:'column'}}>
      ...
      <div style={{flex:1, padding:'28px 24px', background:T.cream, color:T.ink, borderRadius:'24px 24px 0 0', marginTop:-12, position:'relative', zIndex:2}}>
        ...
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginTop:6, marginBottom:20}}>
          ...
          <a style={{fontFamily:T.fBn, fontSize:12, color:T.teal2, fontWeight:700}}>পিন ভুলে গেছেন?</a>
        </div>
        ...
        <div style={{marginTop:16, padding:'12px 14px', background:'#fff', border:`1px dashed ${T.border2}`, borderRadius:12,
          ...
          🔒 <span style={{color:T.ink2}}>PKSF-সুরক্ষিত।</span> আপনার তথ্য শুধু আপনার PO-তেই থাকবে, অন্য কোথাও যাবে না।
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{marginBottom:16}}>
      ...
      <div style={{
        ...
        boxShadow:'0 1px 2px rgba(0,0,0,0.03)', minWidth:0,
      }}>
        ...
        <input value={value} onChange={e=>onChange(e.target.value)} type={type} placeholder={placeholder}
          style={{flex:1, width:'100%', minWidth:0, padding:'12px 12px', border:'none', outline:'none',
...
function PrimaryBtn({label, onClick, disabled, small, icon, variant='teal'}) {
  ...
  return (
    <button onClick={disabled?null:onClick} style={{
      ...
      boxShadow: disabled?'none':`0 4px 14px ${bg}50`,
...
function OtpScreen({ ctx }) {
  ...
  return (
    <div style={{background:T.cream, minHeight:'100%', padding:'24px'}}>
      ...
      <div style={{padding:'24px 6px 0'}}>
        ...
        <div style={{display:'flex', gap:8, justifyContent:'space-between', marginBottom:24}}>
          {otp.map((d,i)=>(
            <input key={i} ref={el=>refs.current[i]=el} value={d} onChange={e=>set(i,e.target.value)}
              style={{
                width:48, height:56, textAlign:'center', fontFamily:T.fMono, fontSize:22, fontWeight:700,
        ...
        <div style={{textAlign:'center', fontFamily:T.fBn, fontSize:12, color:T.ink3, marginBottom:20}}>
          {timer>0 ? <>পুনরায় পাঠান <span style={{fontFamily:T.fMono,color:T.teal2,fontWeight:700}}>{bn(timer)}s</span></> : <a style={{color:T.teal2,fontWeight:700}}>কোড পুনরায় পাঠান</a>}
        </div>
...
window.FreyaOrb = FreyaOrb;
window.AntariousLogo = AntariousLogo;
