// screens1.jsx — Dashboard, Intake, History, Manager analytics
  ...
  return (
    <div style={{background:T.cream, minHeight:'100%'}}>
      ...
      <div style={{
        ...
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:18, position:'relative'}}>
          ...
          <div style={{width:40, height:40, borderRadius:'50%', background:T.teal, color:T.navy,
            display:'flex', alignItems:'center', justifyContent:'center', fontFamily:T.fHead, fontSize:17, fontWeight:900}}>ক</div>
        </div>
      ...
      <div style={{margin:'-48px 16px 16px', background:'#fff', borderRadius:18, padding:16,
        ...
        <div style={{
          width:52, height:52, borderRadius:14, background:T.tealBg,
          display:'flex', alignItems:'center', justifyContent:'center',
          fontSize:24, color:T.teal2,
        }}>＋</div>
        <div style={{flex:1}}>
      ...
      <div style={{padding:'4px 16px 20px'}}>
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'baseline', padding:'12px 4px 10px'}}>
          ...
          <a onClick={()=>ctx.go('history')} style={{fontFamily:T.fBn, fontSize:11, color:T.teal2, fontWeight:700, cursor:'pointer'}}>সবগুলো দেখুন →</a>
        </div>
        {applicants.map(a => <ApplicantRow key={a.id} row={a} onClick={()=>{
          ctx.setApplicant(a.id);
          if (a.status === 'completed') ctx.go('result');
          else ctx.go('assessment');
        }}/>)}
      </div>
...
function ApplicantRow({ row, onClick }) {
  ...
  const color = row.status==='completed' ? (row.rating==='A'?T.green:row.rating==='B'?T.teal:row.rating==='C'?T.amber:T.coral) : T.gold;
  return (
    <div onClick={onClick} style={{
      ...
      <div style={{
        width:44, height:44, borderRadius:12, background:p.tint, color:'#fff',
        display:'flex', alignItems:'center', justifyContent:'center',
        fontFamily:T.fHead, fontSize:19, fontWeight:900, flexShrink:0,
      }}>{p.avatar}</div>
      <div style={{flex:1, minWidth:0}}>
        ...
        <div style={{fontFamily:T.fBn, fontSize:11, color:T.ink3, marginBottom:3}}>
      ...
      <div style={{textAlign:'right'}}>
        {row.status==='completed' ? (
          <>
            ...
            {row.flags>0 && <Chip color={T.coral} size={8} style={{marginTop:5}}>⚑ {row.flags} flag{row.flags>1?'s':''}</Chip>}
          </>
        ) : (
          <>
...
function BottomNav({ ctx, active }) {
  ...
  return (
    <div style={{
      ...
      borderTop:`1px solid ${T.border}`, padding:'8px 10px 10px',
...
function IntakeScreen({ ctx }) {
  const p = PERSONAS[ctx.applicantId] || PERSONAS.nasrin;
  const [persona, setPersona] = uS1(ctx.applicantId || 'nasrin');
  const active = PERSONAS[persona];
  uE1(()=>{ ctx.setApplicant(persona); },[persona]);

  return (
    <div style={{background:T.cream, minHeight:'100%'}}>
      <BrandHeader title={{bn:'নতুন আবেদনকারী', en:'New Applicant Intake'}}
        onBack={()=>ctx.go('dashboard')}
        ...
        <div style={{display:'flex', gap:8, marginBottom:16}}>
          {Object.values(PERSONAS).map(pp => (
            <button key={pp.id} onClick={()=>setPersona(pp.id)} style={{
              ...
              display:'flex', alignItems:'center', gap:6, fontFamily:T.fBn, fontSize:11, fontWeight:700,
              color:T.ink, boxShadow: persona===pp.id?`0 0 0 3px ${pp.tint}20`:'none',
            }}>
      ...
      <div style={{padding:'4px 16px 16px'}}>
        ...
        <div style={{background:'#fff', border:`1px solid ${T.border}`, borderRadius:16, padding:12, marginBottom:12}}>
          ...
          <div style={{display:'flex', gap:8}}>
            <div style={{flex:1, aspectRatio:'3/4', minWidth:0, background:`linear-gradient(135deg, ${active.tint}30, ${active.tint}10)`,
              ...
              <div style={{width:48, height:48, borderRadius:'50%', background:active.tint, color:'#fff',
                display:'flex', alignItems:'center', justifyContent:'center', fontFamily:T.fHead, fontSize:22, fontWeight:900}}>{active.avatar}</div>
              <div style={{fontFamily:T.fMono, fontSize:8, color:T.ink3, letterSpacing:0.8}}>APPLICANT PHOTO</div>
            </div>
            <div style={{flex:1, aspectRatio:'3/4', minWidth:0, background:'#fff', border:`1.5px dashed ${T.border2}`, borderRadius:12,
              display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:5, padding:8, textAlign:'center', overflow:'hidden'}}>
        ...
        <div style={{display:'flex', gap:8}}>
          <div style={{flex:1, minWidth:0}}><Field label={{bn:'বয়স', en:'Age'}} value={String(active.age)} onChange={()=>{}}/></div>
          <div style={{flex:1, minWidth:0}}><Field label={{bn:'লিঙ্গ', en:'Gender'}} value={active.gender==='F'?'মহিলা':'পুরুষ'} onChange={()=>{}}/></div>
        </div>
        ...
        <div style={{display:'flex', gap:8}}>
          <div style={{flex:1, minWidth:0}}><Field label={{bn:'ঋণ চাওয়া', en:'Loan ask'}} value={`${bn(active.loanAsk.toLocaleString('en-IN'))} ৳`} onChange={()=>{}}/></div>
          <div style={{flex:1, minWidth:0}}><Field label={{bn:'সঞ্চয়', en:'Savings'}} value={`${bn(active.savings.toLocaleString('en-IN'))} ৳`} onChange={()=>{}}/></div>
        </div>
        <Field label={{bn:'ঋণের উদ্দেশ্য', en:'Loan purpose'}} value={active.loanPurpose} onChange={()=>{}}/>

        {/* Antarious advisory callout */}
        <div style={{
          background:T.tealBg, border:`1px solid ${T.teal}40`, borderLeft:`3px solid ${T.teal}`,
...
function HistoryScreen({ ctx }) {
  const records = [
    { id:'nasrin', score:742, rating:'B', flags:1, date:'২৫ এপ্রিল, ২০২৬', dateEn:'25 Apr 2026', outcome:'approved' },
    { id:'shima',  score:821, rating:'A', flags:0, date:'২৪ এপ্রিল, ২০২৬', dateEn:'24 Apr 2026', outcome:'approved' },
    { id:'rafiq',  score:487, rating:'D', flags:3, date:'২২ এপ্রিল, ২০২৬', dateEn:'22 Apr 2026', outcome:'declined' },
    { id:'nasrin', score:710, rating:'B', flags:0, date:'১৮ এপ্রিল, ২০২৬', dateEn:'18 Apr 2026', outcome:'approved' },
    { id:'shima',  score:625, rating:'C', flags:2, date:'১৫ এপ্রিল, ২০২৬', dateEn:'15 Apr 2026', outcome:'review' },
  ];
    ...
    <div style={{background:T.cream, minHeight:'100%'}}>
      ...
      <div style={{padding:'14px 16px 8px'}}>
        <div style={{display:'flex', gap:6, overflowX:'auto', marginBottom:12}}>
          {[
            {id:'all', bn:'সব', en:'All', n:records.length},
            {id:'approved', bn:'অনুমোদিত', en:'Approved', n:3},
            {id:'review', bn:'পুনর্বিবেচনা', en:'Review', n:1},
            {id:'declined', bn:'প্রত্যাখ্যাত', en:'Declined', n:1},
          ].map(f=>(
            <button key={f.id} onClick={()=>setFilter(f.id)} style={{
              padding:'7px 13px', borderRadius:20, border:'none',
              background: filter===f.id?T.navy:'#fff',
              color: filter===f.id?'#fff':T.ink2, cursor:'pointer',
              fontFamily:T.fBn, fontSize:11, fontWeight:700, whiteSpace:'nowrap',
          ...
          return (
            <div key={i} onClick={()=>{ctx.setApplicant(r.id); ctx.go('result');}} style={{
              ...
              <div style={{width:44, height:44, borderRadius:12, background:p.tint, color:'#fff',
                display:'flex', alignItems:'center', justifyContent:'center',
                fontFamily:T.fHead, fontSize:19, fontWeight:900, flexShrink:0}}>{p.avatar}</div>
              <div style={{flex:1}}>
...
function AnalyticsScreen({ ctx }) {
  ...
  const monthly = [45,58,62,71,68,82,91];
  return (
    <div style={{background:T.cream, minHeight:'100%'}}>
      ...
      <div style={{padding:'14px 16px 16px'}}>
        ...
        <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:12}}>
          {[
            ...
            {bn:'মিথ্যা পতাকা', en:'Lie-flags raised', val:'১৯', delta:'১৩%', color:T.coral},
          ].map((k,i)=>(
        ...
        <div style={{background:'#fff', border:`1px solid ${T.border}`, borderRadius:14, padding:'14px 14px 16px', marginBottom:12}}>
          ...
          <div style={{display:'flex', alignItems:'flex-end', gap:6, height:92, padding:'0 2px'}}>
            {monthly.map((v,i)=>(
              <div key={i} style={{flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:4}}>
                <div style={{width:'100%', height:`${v}%`, background:`linear-gradient(180deg, ${T.teal}, ${T.teal2})`, borderRadius:'6px 6px 0 0', position:'relative'}}>
                  <span style={{position:'absolute', top:-14, left:'50%', transform:'translateX(-50%)',
                    fontFamily:T.fMono, fontSize:8.5, color:T.ink3, fontWeight:700}}>{bn(v)}</span>
                </div>
        ...
        {/* Dim breakdown */}
        <div style={{background:'#fff', border:`1px solid ${T.border}`, borderRadius:14, padding:'14px 14px 14px', marginBottom:12}}>
          ...
          {dimAvg.map(d=>(
            <div key={d.id} style={{marginBottom:10}}>
              ...
              <div style={{height:6, background:T.cream2, borderRadius:3, overflow:'hidden'}}>
                <div style={{width:`${d.pct}%`, height:'100%', background:d.color, borderRadius:3, transition:'width 0.6s'}}/>
              </div>
            </div>
          ))}
        </div>
